from tkinter import *
import os
from PIL import ImageTk, Image  #PIL means Python Imaging Library
    ...
    for name in accounts:
        if name == username:
            ...
            if isPassword == password:
                ...
                accDetailsWindow.resizable(FALSE, FALSE)
                ...
                Button(accDetailsWindow, text="Personal Details", font=("Times", 12), width=15, height=1, command=personal).grid(column=0, columnspan=2, pady=4)
...
def click_forgot():
    global a1
    global username
    a1 = Label(forgotpasswordWindow)
    a1.grid(column=0, columnspan=2, pady=10)
    ...
    for name in accounts:
        if name == username:
            ...
            if forgetpassword == entry.get():
                ...
                Label(accDetailsWindow, text='Account Details', justify=CENTER, font=("Times", 22, "bold italic")).grid(
                    columnspan=5, padx=10)
                Label(accDetailsWindow, text="Welcome to MyBank", justify=CENTER, font=("Times", 16)).grid(pady=10,
                                                                                                           padx=10)
                Label(accDetailsWindow, text='Select your choice:', justify=LEFT, font=("Times", 13)).grid(sticky=W,
                                                                                                           padx=10)
                Label(accDetailsWindow, text="").grid()
                Button(accDetailsWindow, text="Personal", font=("Times", 12), width=15, height=1,
                       command=personal).grid(column=0, columnspan=2, pady=4)
                Button(accDetailsWindow, text="Deposit", font=("Times", 12), width=15, height=1, command=deposit).grid(column=0, columnspan=2, pady=4)
                Button(accDetailsWindow, text="Withdraw", font=("Times", 12), width=15, height=1,
                       command=withdraw).grid(column=0, columnspan=2, pady=4)
                Button(accDetailsWindow, text="Transfer", font=("Times", 12), width=15, height=1,
                       command=transfer).grid(column=0, columnspan=2, pady=4)
                return
            else:
...
def personal():
    ...
    personalDetailsWindow.resizable(FALSE, FALSE)
...
def done_deposit():
    ...
    l1.config(text="Current Balance:\t\t\u20B9 " + str(new_balance), fg="purple")
...
def withdraw():
    ...
    withdrawWindow.resizable(FALSE, FALSE)
    ...
    l2.grid(row=3, column=1, sticky=W, padx=10)
    e2 = Entry(withdrawWindow, width=15, borderwidth=2, textvariable=amt2)
...
def done_withdraw():
    ...
    accDetails = data.split('\n')
    old_balance = accDetails[7]
    if amt2.get() == "" or float(amt2.get()) <= 0:
        l3.config(text="*Amount cannot be withdrawn*\n*Please enter proper figure*", fg="red")
        return

    elif float(old_balance) < float(amt2.get()):
        l3.config(text="*Amount cannot be withdrawn*\n*Insufficient Balance*", fg="red")
        return

    else:
        new_balance = float(old_balance) - float(amt2.get())
        data = data.replace(old_balance, str(new_balance))
        file.seek(0)
        file.truncate(0)
        file.write(data)
...
def transfer():
    ...
    data = file.read()
    accDetails = data.split('\n')
    l1 = Label(transferWindow, text="Current Balance:\t\t\u20B9" + accDetails[7])
    l1.grid(row=2, column=1, sticky=W, padx=10)
    ...
    e2.grid(row=3, column=2, sticky=W, padx=10)
    l2 = Label(transferWindow, text="Amount to tranfer:\t\u20B9")
    l2.grid(row=4, column=1, sticky=W, padx=10)
    e1 = Entry(transferWindow, width=15, borderwidth=2, textvariable=amt3)
...
def done_transfer():
    ...
    for names in accounts:
        if names == recipient:
            ...
            new_balance = float(new_balance) - float(amt3.get())
            if amt3.get() == "" or float(amt3.get()) <= 0:
                l4.config(text="Amount cannot be transfered\nPlease enter proper figure", fg="red")
                return
            if float(amt3.get()) > float(old_balance):
                l4.config(text="Amount cannot be transfered\nInsufficient Balance", fg="red")
                return
